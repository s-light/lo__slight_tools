# -*- coding: utf-8 -*-
"""
Save the Active Calc sheet as pdf.
...
def get_sheet_pdf_fileUrl(doc):
    ...
    pdf_filename_full_path = os.path.join(path_full, file_basename_new) + ".pdf"
    print("pdf_filename_full_path:", pdf_filename_full_path)
    pdf_fileUrl = uno.systemPathToFileUrl(pdf_filename_full_path)

    return pdf_fileUrl


def save_active_sheet_as_pdf(call_args=[]):
    """Save the Active Document with new current timeStamp."""
    print("-" * 42)
    print("save_active_sheet_as_pdf:")
    ...
    pdf_options = {
        # "PageRange": "5",
        "Selection": sheet_active,
        # "Overwrite": True,
    }

    filters = {
        "FilterName": "calc_pdf_Export",
        "FilterData": dict_to_property(pdf_options, True),
    }

    args = dict_to_property(filters)

    pdf_fileUrl = get_sheet_pdf_fileUrl(doc)
    print("pdf_fileUrl:", pdf_fileUrl)
    # https://wiki.openoffice.org/wiki/Documentation/DevGuide/OfficeDev/Storing_Documents
    try:
        doc.storeToURL(pdf_fileUrl, args)
...
g_exportedScripts = (save_active_sheet_as_pdf,)
